...
const LiveAuction = () => {
  ...
  const fetchLiveAuction = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://23.101.29.218:5000/api/auction/live/bidder', {
        headers: {
  ...
  const placeBid = useCallback(async (auctionId, amount) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://23.101.29.218:5000/api/bid/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const fetchBidderRank = useCallback(async (auctionId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://23.101.29.218:5000/api/bid/rank?auction_id=${auctionId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
...
const handleRefresh = async () => {
  ...
  const fetchLatestBid = useCallback(async (auctionId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://23.101.29.218:5000/api/bid/latest?auction_id=${auctionId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
